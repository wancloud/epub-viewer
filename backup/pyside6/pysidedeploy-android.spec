...
# NOTE: leaving this blank does NOT auto-download the NDK for a pre-existing spec file
# like this one (verified against android_config.py's source -- that fallback only runs
# for a spec freshly generated by --init). The CI workflow downloads the required NDK
# version itself and passes it via `pyside6-android-deploy --ndk-path ...`, which
# overrides this field, so it's fine to leave blank for CI. For a manual/local build,
# either fill in a real NDK path here or pass --ndk-path on the command line.
ndk_path =
